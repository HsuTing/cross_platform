package hw2_F74011140_V01;
import java.lang.Math;

public class hero_C extends hero{
	public void sethero_C(){
		setHP();
		setMP();
		setPhysical_ATK();
		setPhysical_DEF();
		setMagic_ATK();
		setMagic_DEF();
		setPhysical_Vampire();
		setMagic_Vampire();
		getOriginal();
	}
	
	public void getOriginal(){
		System.out.println("英雄 C");
		System.out.printf("體力值 %.2f\n", this.HP);
		System.out.printf("魔力值 %.2f\n", this.MP);
		System.out.printf("物理攻擊 %.2f\n", this.Physical_ATK);
		System.out.printf("魔法攻擊 %.2f\n", this.Magic_ATK);
		System.out.printf("物理防禦 %.2f\n", this.Physical_DEF);
		System.out.printf("魔法防禦 %.2f\n", this.Magic_DEF);
		System.out.printf("物理吸血 %.2f\n", this.Physical_Vampire);
		System.out.printf("魔法吸血 %.2f\n", this.Magic_Vampire);
		System.out.println("技能：");
		System.out.println("群星殞落 魔法攻擊提高100傷害 / 消耗魔力 40");
		System.out.println("星之祝福 回復體力200點 / 消耗魔力 80");
		System.out.println("魔力灌注 魔法攻擊提高5%魔力值的傷害 / 消耗魔力 20");
	}
	
	public void getInformation(){
		System.out.printf("英雄 C 體力 %.2f 魔力%.2f\n", this.HP, this.MP);
	}
	
	public double getHP(){
		return this.HP;
	}
	
	public double getMP(){
		return this.MP;
	}

	public void setHP(){
		HP = (Math.random() * 100 + 400);
	};
	
	public void setMP(){
		MP = (Math.random() * 100 + 1000);
	};
	
	public void setPhysical_ATK(){
		Physical_ATK = 0;
	};
	
	public void setPhysical_DEF(){
		Physical_DEF = (Math.random() * 5 + 5);
	};
	
	public void setMagic_ATK(){
		Magic_ATK = (Math.random() * 10 + 40);
	};
	
	public void setMagic_DEF(){
		Magic_DEF = (Math.random() * 10 + 30);
	};
	
	public void setPhysical_Vampire(){
		Physical_Vampire = 0;
	};
	
	public void setMagic_Vampire(){
		Magic_Vampire = (Math.random() * 5 + 10);
	};
	
	public hero skill(hero a){
		int choose = (int)(Math.random() * 10 + 0);
		
		switch(choose){
			case 1:
			case 2:
			case 3:
				if(this.MP - 40 > 0){
					System.out.printf("英雄C 群星殞落 %.2f;", ( this.Magic_ATK + 100 ) / ( 1 - a.Magic_DEF / 100 ));
					a.HP = a.HP - ( this.Magic_ATK + 100 ) / ( 1 - a.Magic_DEF / 100 );
					this.MP = this.MP - 40;
				}
				else{
					System.out.printf("英雄C 魔法攻擊 %.2f;", this.Magic_ATK);
					a.HP = a.HP - this.Magic_ATK * ( 1 - a.Magic_DEF / 100 );
					this.HP = this.HP + this.Magic_Vampire;
				}
				break;
			case 4:
			case 5:
				if(this.MP - 80 > 0){
					System.out.printf("英雄C 星之祝福 %.2f;", 200);
					this.HP = this.HP + 200;
					this.MP = this.MP - 80;
				}
				else{
					System.out.printf("英雄C 魔法攻擊 %.2f;", this.Magic_ATK);
					a.HP = a.HP - this.Magic_ATK * ( 1 - a.Magic_DEF / 100 );
					this.HP = this.HP + this.Magic_Vampire;
				}
				break;
			case 6:
			case 7:
				if(this.MP - 20 > 0){
					System.out.printf("英雄C 魔力灌注 %.2f;", ( this.Magic_ATK + 0.05 * this.MP ) * ( 1 - a.Magic_DEF / 100 ));
					a.HP = a.HP - ( this.Magic_ATK + 0.05 * this.MP ) * ( 1 - a.Magic_DEF / 100 );
					this.MP = this.MP - 20;
				}
				else{
					System.out.printf("英雄C 魔法攻擊 %.2f;", this.Magic_ATK);
					a.HP = a.HP - this.Magic_ATK * ( 1 - a.Magic_DEF / 100 );
					this.HP = this.HP + this.Magic_Vampire;
				}
				break;
			default:
				System.out.printf("英雄C 魔法攻擊 %.2f;", this.Magic_ATK);
				a.HP = a.HP - this.Magic_ATK * ( 1 - a.Magic_DEF / 100 );
				this.HP = this.HP + this.Magic_Vampire;
				break;
		}
		
		return a;
	};
}